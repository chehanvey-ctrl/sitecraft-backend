from fastapi import FastAPI from fastapi.middleware.cors import CORSMiddleware from pydantic import BaseModel import openai import os
...
@app.post("/generate-pure") async def generate_pure_site(request: PromptRequest): prompt = request.prompt
...
try:
    html_response = openai.chat.completions.create(
        ...
        messages=[
            ...
            {"role": "user", "content": f"Create a beautiful one-page responsive website using only HTML and embedded CSS. Include a full-width hero image using this image URL: {image_url}.\n\nThe site should include:\n- An H1 title inspired by this prompt: {prompt}\n- A tagline under the title\n- At least five content sections\n- A footer\nUse elegant layout and matching colors. Return only valid HTML."}
        ],
        temperature=0.7,
        max_tokens=1800
    )
